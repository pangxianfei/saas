...
import "gitee.com/pangxianfei/simple/sqlcmd"
...
func JsonPageData(results interface{}, page *sqlcmd.Paging) *JsonResult {
	return JsonData(&sqlcmd.PageResult{
		Results: results,
		Page:    page,
	})
}
...
func JsonCursorData(results interface{}, cursor string, hasMore bool) *JsonResult {
	return JsonData(&sqlcmd.CursorResult{
		Results: results,
		Cursor:  cursor,
		HasMore: hasMore,
	})
...
func JsonError(data interface{}) *JsonResult {
	return &JsonResult{
		Code:    401,
		Message: "提交失败",
		Data:    data,

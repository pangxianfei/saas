...
import (
	...
	"gitee.com/pangxianfei/saas/tenantglobal"
...
func (app *App) Initiation(ctx iris.Context) (TenantDB *gorm.DB) {
	TenantsId := ctx.Values().Get("TenantId").(int64)
	AppId := ctx.Values().Get("AppId").(int64)
	return app.SetTenantsDb(TenantsId, AppId)
}
func (app *App) SetTenantsDb(TenantsId int64, AppId int64) (TenantDB *gorm.DB) {
	...
	UserTenantsDB, hasDbConn := tenantglobal.InstanceDBMap.Load(TenantsAppDBNo)
	if hasDbConn {
		...
		if ThemTenantsDB != nil && ThemTenantsDB.TenantsId == TenantsId && ThemTenantsDB.AppId == AppId && CurrInstanceDB.TenantsId == TenantsId && CurrInstanceDB.AppId == AppId {
			app.OutConsole("缓存", 1, ThemTenantsDB.TenantsId, ThemTenantsDB.AppId, ThemTenantsDB.DbName)
	...
	app.OutConsole("出始化", 3, CurrInstanceDB.TenantsId, CurrInstanceDB.AppId, CurrInstanceDB.DbName)
	return app.SetSourceData(CurrInstanceDB)
}

func (app *App) SetSourceData(CurrInstanceDB *sysmdel.InstanceDb) (InstanceDB *gorm.DB) {
	var mysqlStr string
	var dber *sql.DB
	...
	tenantglobal.InstanceDBMap.Store(TenantsAppDBNo, &SaveConnectionObject)
	return InstanceDB
}

func (app *App) MysqlOpenDb(Instance *sysmdel.InstanceDb) (TenantDB *gorm.DB) {
	var DbDns string
	//如果是使用外网IP
...
func (app *App) OutConsole(title string, serial int64, TenantsId int64, AppId int64, DbName string) {
